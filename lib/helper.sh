...
# Quiets any PIPEFAIL when 1 (default: 0)
#
: "${PIPEFAIL=0}"
...
#######################################
has() {
  local all=false executable="" image="" path=false r rv value=false
...
#######################################
stack() {
  local c color file frame i line symbol
...
_strict() {
  local code=$? diff funcname last _tmp
  set +x
  _tmp="$(mktemp)"
...
strict() {
  ...
  if [ "${BASH_VERSION-}" ]; then
    ...
    set "${_set:-+o}" errtrace
    [ "${PIPEFAIL-0}" -eq 0 ] || set "${_set:-+o}" pipefail
